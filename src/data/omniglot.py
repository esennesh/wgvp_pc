...
class OmniglotDataModule(datamodule.DataModule):
    def __init__(self, *args, binarize=True, side=28, **kwargs):
        trnsfrms = [
            transforms.ToTensor(),
            transforms.Resize((side, side)),
            transforms.Lambda(
                lambda x: 1. - np.array(x.numpy(), dtype=jnp.float32)
            ),
    ...
    def prepare_data(self) -> Tuple[Dataset, Dataset]:
        data_train = datasets.Omniglot(self.data_dir, background=True,
                                       download=True, transform=self.transforms)
        data_test = datasets.Omniglot(self.data_dir, background=False,
                                      download=True, transform=self.transforms)
